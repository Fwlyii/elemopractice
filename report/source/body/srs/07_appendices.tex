...
具体接口路径、数据表名、测试类/方法和最终结果在接口文档、数据库文档和测试报告中按需求编号补全，SRS 不绑定具体 Java 类名。

\subsection{单项需求完成标准与配套交付物}

一项 P0/P1 需求仅在需求与业务规则已确认、接口和权限行为已同步、正常/异常/边界测试具有明确断言、前后端联调通过、受影响的回归测试通过且主责成员能够解释实现和测试结果时标记为完成。

SRS 后续应与数据库设计、REST 接口文档、自动化测试与覆盖率报告、需求变更记录、AI 辅助使用记录、部署说明和交叉验收记录保持需求编号一致。

\subsection{需求与模板对应关系}

本文第 1--8 节与《UML 大战需求分析》附录 1 的需求规格说明书结构对应。为适应课程验收，本文在附录中增加需求追踪矩阵、可执行验收场景和单项需求完成标准；这些内容属于模板的补充材料，不改变正文的八节结构。
